...
static unsigned char mac3[6]={0x00,0x1a,0xca,0x00,0xd2,0xa0};
//static unsigned char mac4[6]={0x00,0x1a,0xca,0x00,0xd2,0xa1};
static unsigned char macb[6]={0xff,0xff,0xff,0xff,0xff,0xff};
...
static void* main_aux(void* arg)
{
	...
	while (local_total < num_packets || num_packets < 0)
	{
		...
		long slot = gxio_mpipe_equeue_reserve_fast(equeue, 1);
	        //mydelay(9000,1);
		// Egress the packets.
		for (int i = 0; i < n; i++)
		{
		...
		printf("totally forward %d packets.\n",(int)local_total);
...
main(int argc, char** argv)
{
	...
	char* link_name_in = "xgbe2";
	char* link_name_out = "xgbe3";
